...
最后调用 `insert` 方法把 DOM 插入到父节点中，因为是递归调用，子元素会优先调用 `insert`，所以整个 `vnode` 树节点的插入顺序是**先子后父**。来看一下 `insert` 方法，它的定义在 src/core/vdom/patch.js 上。
...
![new Vue](../assets/img/new-vue.png)
